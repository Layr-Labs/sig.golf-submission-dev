...
namespace SigGolfCandidate.Hypertree.GroupedBalancedPrivateDerivation67
open SigGolf OracleSpec OracleComp SigGolfCandidate.Hypertree Reference
open SecurityRandomOracle SecuritySecretKey GroupedBalancedAddressDomains67

inductive Slot where
  | bottom (index : BitVec 160)
  | upper (base : Fin 150) (leaf : BitVec 160) (pair : Fin 34)
...
theorem eval_real_upper (hash : Hash) (secretKey : SecretKey)
    ...
    evalWithAnswerFn hash (realDerivation secretKey (.upper base leaf pair)) =
      GroupedBalancedUpperTree67.secretPair hash secretKey (base.val + 10) leaf.toNat pair.val := rfl
...
end SigGolfCandidate.Hypertree.GroupedBalancedPrivateDerivation67
