import SigGolfCandidate.Hypertree.GroupedBalancedSignByteBranchSteps67
import SigGolfCandidate.Hypertree.GroupedBalancedChecksum67

/-! Inlined from SigGolfCandidate.Hypertree.GroupedBalancedSignByteTail67; its only importer was SigGolfCandidate.Hypertree.GroupedBalancedSignByteCompose67. -/
section
...
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignByteTail67
...
private theorem word1123 : GroupedBalancedSignImage67Byte.code[1123]? =
    some 0x00900c93 := by decide
private theorem word1124 : GroupedBalancedSignImage67Byte.code[1124]? =
    some 0x039bfd33 := by decide
private theorem word1125 : GroupedBalancedSignImage67Byte.code[1125]? =
    some 0x039bddb3 := by decide
private theorem word1126 : GroupedBalancedSignImage67Byte.code[1126]? =
    some 0x01a580a3 := by decide
private theorem word1127 : GroupedBalancedSignImage67Byte.code[1127]? =
    some 0x01b58123 := by decide
private theorem word1128 : GroupedBalancedSignImage67Byte.code[1128]? =
    some 0x00008067 := by decide

theorem tail_steps (s : MachineState) (pc : s.pc = 0x218c)
    (valid1 : accessValid (s.getReg .x11 + 1) 1 = true)
    (valid2 : accessValid (s.getReg .x11 + 2) 1 = true) :
    OrdinarySteps GroupedBalancedSignImage67Byte.image s 6 (tailState s) := by
  ...
  change OrdinarySteps GroupedBalancedSignImage67Byte.image s 6 s6
  apply OrdinarySteps.step s s1 _ (.base (.ADDI .x25 .x0 9)) 5
  · simp [fetch, pc, GroupedBalancedSignImage67Byte.image, word1123]; rfl
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.REMU .x26 .x23 .x25)) 4
  · have hp : s1.pc = 0x2190 := by simp [s1, execInstrBr, pc]
    simp [fetch, hp, GroupedBalancedSignImage67Byte.image, word1124]; rfl
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.DIVU .x27 .x23 .x25)) 3
  · have hp : s2.pc = 0x2194 := by simp [s1, s2, execInstrBr, pc]
    simp [fetch, hp, GroupedBalancedSignImage67Byte.image, word1125]; rfl
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.SB .x11 .x26 1)) 2
  · have hp : s3.pc = 0x2198 := by simp [s1, s2, s3, execInstrBr, pc]
    simp [fetch, hp, GroupedBalancedSignImage67Byte.image, word1126]; rfl
  ...
  · have hp : s4.pc = 0x219c := by simp [s1, s2, s3, s4, execInstrBr, pc]
    simp [fetch, hp, GroupedBalancedSignImage67Byte.image, word1127]; rfl
  ...
  · have hp : s5.pc = 0x21a0 := by simp [s1, s2, s3, s4, s5, execInstrBr, pc]
    simp [fetch, hp, GroupedBalancedSignImage67Byte.image, word1128]; rfl
  · rfl
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignByteTail67
...
open SigGolfCandidate.Hypertree.GroupedBalancedSignByteBranchSteps67
open SigGolfCandidate.Hypertree.GroupedBalancedSignByteTail67
set_option maxRecDepth 8192
set_option maxHeartbeats 0

namespace SigGolfCandidate.Hypertree.GroupedBalancedSignByteCompose67
...
theorem copyLoop_preserves_flag (s : MachineState)
  ...
  | succ n ih =>
    ...
    change (GroupedBalancedSignByteCopy67.copyBody (copyLoop s n)).getByte 0x80640 = _
...
theorem selectedBranch_fields (s : MachineState) (message : BitVec 128)
    ...
    (selectedBranch s message).pc = 0x2168 ∧
...
theorem postSum_payload_bytes (s : MachineState) (message : BitVec 128)
  ...
  exact GroupedBalancedSignBytePayload67.output_payload_bytes b message
...
theorem copyBody_x23 (s : MachineState) :
    (GroupedBalancedSignByteCopy67.copyBody s).getReg .x23 = s.getReg .x23 := by
  simp [GroupedBalancedSignByteCopy67.copyBody, execInstrBr,
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignByteCompose67
