...
theorem loop_block (s : MachineState) (pc : s.pc = 0x1018)
    ...
    OrdinarySteps image s 6 (loopNext s) := by
  ...
  · simp only [fetch, s1, s2, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.ADDI .x7 .x7 8)) 2
  · simp only [fetch, s1, s2, s3, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.ADDI .x10 .x10 (-1))) 1
  · simp only [fetch, s1, s2, s3, s4, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s5 s6 _ (.base (.BNE .x10 .x0 (-20))) 0
  · simp only [fetch, s1, s2, s3, s4, s5, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
...
theorem finish (hash : Hash) (s : MachineState) (pc : s.pc = 0x1030) :
    Executes hash image s 3 ⟨.success, finishState s, 3, 0, 0⟩ := by
  have block : OrdinarySteps image s 2 (finishState s) := by
    ...
    · simp only [fetch, execInstrBr, MachineState.setPC, pc, image]; decide
    · rfl
    exact OrdinarySteps.refl _
  have hf : fetch image (finishState s) = some (.base .ECALL) := by
    simp only [fetch, finishState, execInstrBr, MachineState.setPC, pc, image]; decide
...
def Invariant (n : Nat) (s : MachineState) : Prop :=
  n ≤ 6356 ∧ s.pc = (if n = 0 then 0x1030 else 0x1018) ∧
  s.getReg .x6 = BitVec.ofNat 64 (0x20060 + 8 * (6356 - n)) ∧
  s.getReg .x7 = BitVec.ofNat 64 (0x2c700 + 8 * (6356 - n)) ∧
...
theorem loop_invariant (n : Nat) (s : MachineState) (inv : Invariant (n + 1) s) :
  ...
  · rw [(loop_next_regs s).1, src]
    change BitVec.ofNat 64 (0x20060 + 8 * (6356 - (n + 1))) + BitVec.ofNat 64 8 = _
    rw [← BitVec.ofNat_add]
    congr 1
    omega
  · rw [(loop_next_regs s).2.1, dst]
    change BitVec.ofNat 64 (0x2c700 + 8 * (6356 - (n + 1))) + BitVec.ofNat 64 8 = _
    rw [← BitVec.ofNat_add]
    congr 1
    omega
...
theorem loop_executes (hash : Hash) (n : Nat) (s : MachineState) (inv : Invariant n s) :
    ∃ final, Executes hash image s (6 * n + 3) ⟨.success, final, 6 * n + 3, 0, 0⟩ := by
...
def prefixState (s : MachineState) : MachineState :=
  ...
  let s := execInstrBr s (.LUI .x7 0x2c)
  let s := execInstrBr s (.ADDI .x7 .x7 0x700)
  let s := execInstrBr s (.LUI .x10 2)
  execInstrBr s (.ADDI .x10 .x10 0x8d4)

theorem prefix_block (s : MachineState) (pc : s.pc = 0x1000) :
    OrdinarySteps image s 6 (prefixState s) := by
  let s1 := execInstrBr s (.LUI .x6 0x20)
  let s2 := execInstrBr s1 (.ADDI .x6 .x6 0x60)
  let s3 := execInstrBr s2 (.LUI .x7 0x2c)
  let s4 := execInstrBr s3 (.ADDI .x7 .x7 0x700)
  let s5 := execInstrBr s4 (.LUI .x10 2)
  let s6 := execInstrBr s5 (.ADDI .x10 .x10 0x8d4)
  ...
  · simp only [fetch, s1, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.LUI .x7 0x2c)) 3
  · simp only [fetch, s1, s2, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.ADDI .x7 .x7 0x700)) 2
  · simp only [fetch, s1, s2, s3, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.LUI .x10 2)) 1
  · simp only [fetch, s1, s2, s3, s4, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  apply OrdinarySteps.step s5 s6 _ (.base (.ADDI .x10 .x10 0x8d4)) 0
  · simp only [fetch, s1, s2, s3, s4, s5, execInstrBr, MachineState.setPC, pc, image]; decide
  · rfl
  exact OrdinarySteps.refl _

theorem prefix_invariant (s : MachineState) (pc : s.pc = 0x1000) :
    Invariant 6356 (prefixState s) := by
  unfold Invariant prefixState
  simp [execInstrBr, MachineState.getReg_setReg_ne, MachineState.getReg_setReg_eq,
    signExtend12, pc]

/-- The actual expansion image succeeds on arbitrary memory contents in exactly 38,145 cycles, without hash calls. -/
theorem executes (hash : Hash) (s : MachineState) (pc : s.pc = 0x1000) :
    ∃ final, Executes hash image s 38145 ⟨.success, final, 38145, 0, 0⟩ := by
  obtain ⟨final, tail⟩ := loop_executes hash 6356 (prefixState s) (prefix_invariant s pc)
  refine ⟨final, ?_⟩
...
theorem run_bound (hash : Hash) (input : Input submission.sizes .expand) :
    ...
    result.finished = true ∧ result.value.isSome = true ∧ result.cycles = 38145 ∧
  ...
  have bound : 38145 ≤ CYCLE_LIMIT := by decide
  have run := runWith_of_executes submission hash .expand input state 38145
    ⟨.success, final, 38145, 0, 0⟩ loaded trace bound
  simp [run]

/-- info: 'SigGolfCandidate.Hypertree.Expansion67.executes' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
