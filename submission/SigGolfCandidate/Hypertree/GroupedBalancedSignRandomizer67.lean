...
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignRandomizer67
...
theorem randomizerHashState_block (s : MachineState) (pc : s.pc = 0x1090) :
    OrdinarySteps image s 6 (randomizerHashState s) := by
  ...
  · have hp : s.pc = 0x1090 := by simp [execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.ADDI .x10 .x10 0)) 4
  · have hp : s1.pc = 0x1094 := by simp [s1, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.ADDI .x11 .x0 768)) 3
  · have hp : s2.pc = 0x1098 := by simp [s1, s2, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.LUI .x12 0x80)) 2
  · have hp : s3.pc = 0x109c := by simp [s1, s2, s3, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.ADDI .x12 .x12 0x300)) 1
  · have hp : s4.pc = 0x10a0 := by simp [s1, s2, s3, s4, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s5 s6 _ (.base (.ADDI .x5 .x0 1)) 0
  · have hp : s5.pc = 0x10a4 := by simp [s1, s2, s3, s4, s5, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
...
theorem randomizerCopyState_block (s : MachineState) (pc : s.pc = 0x10ac) :
    OrdinarySteps image s 5 (randomizerCopyState s) := by
  ...
  · have hp : s.pc = 0x10ac := by simp [execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.ADDI .x6 .x6 0x300)) 3
  · have hp : s1.pc = 0x10b0 := by simp [s1, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.LUI .x7 0x20)) 2
  · have hp : s2.pc = 0x10b4 := by simp [s1, s2, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.ADDI .x7 .x7 0x60)) 1
  · have hp : s3.pc = 0x10b8 := by simp [s1, s2, s3, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.ADDI .x10 .x0 4)) 0
  · have hp : s4.pc = 0x10bc := by simp [s1, s2, s3, s4, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  exact OrdinarySteps.refl _
...
theorem randomizerCopyState_invariant (s : MachineState) (pc : s.pc = 0x10ac) :
    CopyInvariant 0x10c0 0x80300 0x20060 4 4 (randomizerCopyState s) := by
  simp [CopyInvariant, randomizerCopyState, execInstrBr, signExtend12, pc,
    MachineState.getReg_setReg_eq, MachineState.getReg_setReg_ne]

theorem randomizer_copy_code : CopyCode image 0x10c0 := by decide

/-- All four words of the hash answer are copied to the serialized signature prefix. -/
theorem randomizer_copy (s : MachineState) (pc : s.pc = 0x10ac) :
    ∃ final, OrdinarySteps image s 29 final ∧ final.pc = 0x10d8 ∧
      ∀ i, i < 4 → final.getMem (wordAddress 0x20060 i) = s.getMem (wordAddress 0x80300 i) := by
  obtain ⟨final, loop, inv, output, _⟩ := copy_all image 0x10c0 randomizer_copy_code
    0x80300 0x20060 4 (randomizerCopyState s) (randomizerCopyState_invariant s pc)
    (by decide) (by decide) (by decide) (by decide) (by decide)
  refine ⟨final, ?_, ?_, ?_⟩
  · exact ordinary_trans image s _ final 5 24 (randomizerCopyState_block s pc) loop
...
theorem randomizer_trace (hash : Hash) (s : MachineState) (pc : s.pc = 0x1090) :
    ∃ final, Trace hash image s 36 51 1 2 final ∧ final.pc = 0x10d8 ∧
  ...
  have hpc : hs.pc = 0x10a8 := by simp [hs, randomizerHashState_pc, pc]
  obtain ⟨service, src, len, dst⟩ := randomizerHashState_regs s
  have hf : fetch image hs = some (.base .ECALL) := by simp only [fetch, hpc]; decide
  ...
  have outpc : (writeHash hs answer).pc = 0x10ac := by simp [hash_pc, hpc]
  obtain ⟨final, copied, finalpc, words⟩ := randomizer_copy (writeHash hs answer) outpc
  have call : Trace hash image hs 1 16 1 2 (writeHash hs answer) := by
...
theorem randomizer_trace_frame (hash : Hash) (s : MachineState) (pc : s.pc = 0x1090) :
    ∃ final, Trace hash image s 36 51 1 2 final ∧ final.pc = 0x10d8 ∧
      (∀ i : Fin 4, final.getMem (wordAddress 0x20060 i.val) =
  ...
  let answer := hash (hashInput hs)
  have outpc : (writeHash hs answer).pc = 0x10ac := by simp [hash_pc, hpc]
  obtain ⟨final, loop, inv, output, frame⟩ := copy_all image 0x10c0 randomizer_copy_code
    0x80300 0x20060 4 (randomizerCopyState (writeHash hs answer))
    (randomizerCopyState_invariant (writeHash hs answer) outpc)
    (by decide) (by decide) (by decide) (by decide) (by decide)
  have copied : OrdinarySteps image (writeHash hs answer) 29 final :=
    ordinary_trans image _ _ final 5 24 (randomizerCopyState_block _ outpc) loop
  have call : Trace hash image hs 1 16 1 2 (writeHash hs answer) := by
    simpa [hlen, compressions] using Trace.hash hs (writeHash hs answer) 0 0 0 0 hf service hv
      (Trace.refl (writeHash hs answer))
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignRandomizer67
