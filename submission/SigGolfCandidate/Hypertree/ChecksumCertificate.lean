...
/-!
Propagate the checksum-aware chain bound through the verifier bytecode.
The in-place chain loop reuses the hash header across chains and the direct leaf
compression skips a 736-byte endpoint copy on every upper layer.
-/
...
theorem recover_all_chains (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    (small : level < 160) (pc : s.pc = 0x1490) (data : LeafData s level tree side base message values)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 33166 ∧ cycles ≤ 3612 ∧ calls ≤ 322 ∧ final.pc = 0x1690 ∧
  ...
  obtain ⟨final, steps, cycles, calls, run, hsteps, hcycles, hcalls, callEq, finalPC, finalCounter,
    finalData, endpoints, finalRA, finalSP, frame⟩ :=
    Verifying.recover_all_chains_fast hash s level tree side base message values
      small pc data counter aligned bound
  have balanced := Verifying.chainPrefixFast_bound_balanced message
  rw [← callEq] at balanced
...
theorem upper_leaf_body (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles (calls+1) (calls+12) final ∧
      steps ≤ 33781 ∧ cycles ≤ 3766 ∧ calls ≤ 322 ∧
...
theorem upper_leaf_call (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    (small : level < 160) (pc : s.pc = 0x1458) (sp : s.getReg .x2 = 0xfffff0)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles (calls+1) (calls+12) final ∧
      steps ≤ 33795 ∧ cycles ≤ 3780 ∧ calls ≤ 322 ∧
...
theorem recover_leaf_call (hash : Hash) (s : MachineState) (level tree base : Nat) (side : Bool)
    ...
    ∃ final steps cycles calls blocks, Trace hash verify s steps cycles calls blocks final ∧
      steps ≤ 33795 ∧ cycles ≤ (if level = 0 then 116 else 3780) ∧ calls ≤ 323 ∧ blocks ≤ 334 ∧
      final.pc = s.getReg .x1 &&& ~~~1#64 ∧ final.getReg .x2 = s.getReg .x2 ∧
      (∀ i : Fin 2, final.getMem (KeygenSavePublic.wordAddress side i.val) =
        (Reference.recoverLeaf hash level tree side message signature).extractLsb' (64*i.val) 64) ∧
      (∀ a, a ≠ 0xffffe0 → OutsideUpperLeaf side a → final.getMem a = s.getMem a) := by
...
theorem recover_tree_call (hash : Hash) (s : MachineState) (level tree base : Nat) (side : Bool)
    ...
    (small : level < 160) (aligned : base % 8 = 0) (bound : base+752 ≤ 0x80000) :
    ∃ final steps cycles calls blocks, Trace hash verify s steps cycles calls blocks final ∧
      steps ≤ 33908 ∧ cycles ≤ (if level = 0 then 235 else 3900) ∧ calls ≤ 324 ∧ blocks ≤ 335 ∧
      final.pc = s.getReg .x1 &&& ~~~1#64 ∧ final.getReg .x2 = s.getReg .x2 ∧
...
theorem verify_layer_tree (hash : Hash) (s : MachineState) (level index : Nat) (side : Bool)
    ...
    ∃ final steps cycles calls blocks, Trace hash verify s steps cycles calls blocks final ∧
      steps ≤ 34405 ∧ cycles ≤ (if level = 0 then 240 else 4397) ∧ calls ≤ 324 ∧ blocks ≤ 335 ∧
      final.pc = 0x11d4 ∧
...
theorem verify_layer (hash : Hash) (s : MachineState) (level index : Nat)
    ...
    (data : LoopData s level index current witness) :
    ∃ final steps cycles calls blocks, Trace hash verify s steps cycles calls blocks final ∧
      steps ≤ 34452 ∧ cycles ≤ (if level = 0 then 286 else 4444) ∧ calls ≤ 324 ∧ blocks ≤ 335 ∧
...
theorem verify_layers (hash : Hash) (witness : Bytes signatureBytes) (count level index : Nat)
    ...
    ∃ final steps cycles calls blocks lastIndex, Trace hash verify s steps cycles calls blocks final ∧
      steps ≤ 34452*count ∧ cycles ≤ 4444*count - (if level = 0 then 4158 else 0) ∧ calls ≤ 324*count ∧ blocks ≤ 335*count ∧
      final.pc = 0x1220 ∧
...
theorem loaded_recovery (hash : Hash) (pk : PublicKey) (message : Message) (witness : Bytes signatureBytes) :
    ∃ initial recovered steps cycles calls blocks,
      ...
      steps ≤ 5512450 ∧ cycles ≤ 707027 ∧ calls ≤ 51841 ∧ blocks ≤ 53602 ∧
      recovered.pc = 0x1220 ∧
  ...
  change cycles ≤ 706882 at hcycles
...
theorem run_refines (hash : Hash) (pk : PublicKey) (message : Message) (witness : Bytes signatureBytes) :
    ∃ cycles calls blocks, cycles ≤ 707042 ∧ calls ≤ 51841 ∧ blocks ≤ 53602 ∧
...
theorem verificationBound : submission.VerificationBound 707042 := by
...
/-- Balanced-digit certificate for the in-place verifier, retaining the cheap bottom-layer bound. -/
theorem certificate : SigGolf.Certificate submission 707042 :=
