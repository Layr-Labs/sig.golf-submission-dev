...
/-!
A height-parametric upper Merkle tree with mixed-radix WOTS leaves. The tree
...
namespace SigGolfCandidate.Hypertree.GroupedBalancedUpperTree67
open SigGolf SigGolfCandidate.Hypertree Reference
set_option maxRecDepth 8192

abbrev ChainMixed := GroupedBalancedChecksum67.Chain
abbrev digit := GroupedBalancedChecksum67.digit
abbrev maxDigit := GroupedBalancedChecksum67.maxDigit
...
def secret (hash : Hash) (secretKey : SecretKey)
    (base leaf : Nat) (chain : ChainMixed) : Digest :=
...
def chainHash (hash : Hash) (base leaf : Nat) (chain : ChainMixed)
...
def compressLeaf (hash : Hash) (base leaf : Nat)
    (values : ChainMixed → Digest) : Digest :=
...
def signValues (hash : Hash) (secretKey : SecretKey)
    (base leaf : Nat) (message : Digest) : ChainMixed → Digest :=
...
def recoverLeaf (hash : Hash) (base leaf : Nat) (message : Digest)
  ...
  compressLeaf hash base leaf (fun chain =>
    walk (chainHash hash base leaf chain) (digit message chain).val
      (maxDigit chain - (digit message chain).val) (values chain))
...
theorem recover_leaf_sign (hash : Hash) (secretKey : SecretKey)
  ...
  congr 1
  funext chain
  have hd : (digit message chain).val ≤ maxDigit chain :=
  ...
  simpa [endpoint, Nat.add_sub_of_le hd] using
    (walk_append (chainHash hash base leaf chain) 0 (digit message chain).val
      (maxDigit chain - (digit message chain).val) (secret hash secretKey base leaf chain)).symm

inductive Witness : Nat → Type where
  | leaf (values : ChainMixed → Digest) : Witness 0
...
theorem Witness.encode_length : {height : Nat} → (witness : Witness height) →
    witness.encode.length = (67 + height) * 16
  ...
  | height + 1, address, index, message =>
      if index / 2 ^ height % 2 = 0 then
        ...
        { root := node hash (base + height) address left.root right,
          ...
          compressions := left.compressions + (266 * 2 ^ height - 1) + 1 }
      else
        ...
        { root := node hash (base + height) address left right.root,
          ...
          compressions := (266 * 2 ^ height - 1) + right.compressions + 1 }
...
end SigGolfCandidate.Hypertree.GroupedBalancedUpperTree67
