...
/-! Inlined from SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureSelectorAfter67; its only importer was SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureDigitAfter67. -/
section
/-! Skip the witness capture unless this is the selected upper leaf. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureSelectorAfter67
...
private theorem selector_code :
    Keygen.instructionAt image 0x1a44 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1a48 = some (.base (.ADDI .x28 .x28 0xe0)) ∧
    Keygen.instructionAt image 0x1a4c = some (.base (.LD .x6 .x28 0)) ∧
    Keygen.instructionAt image 0x1a50 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1a54 = some (.base (.ADDI .x28 .x28 0xe8)) ∧
    Keygen.instructionAt image 0x1a58 = some (.base (.LD .x7 .x28 0)) ∧
    Keygen.instructionAt image 0x1a5c = some (.base (.BNE .x6 .x7 0x50)) := by decide

theorem selector_steps (s : MachineState) (pc : s.pc = 0x1a44) :
...
theorem selector_pc (s : MachineState) (pc : s.pc = 0x1a44) :
    (selectorState s).pc =
      if s.getMem 0x810e0 ≠ s.getMem 0x810e8 then 0x1aac else 0x1a60 := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureSelectorAfter67

end

/-! Compare the selected witness digit against the current H2 step. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureDigitAfter67
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64
open SigGolfCandidate.Hypertree
set_option maxRecDepth 8192
set_option maxHeartbeats 300000
...
private theorem digit_code :
    Keygen.instructionAt image 0x1a60 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1a64 = some (.base (.ADDI .x28 .x28 0x30)) ∧
    Keygen.instructionAt image 0x1a68 = some (.base (.LD .x6 .x28 0)) ∧
    Keygen.instructionAt image 0x1a6c = some (.base (.LUI .x7 0x80)) ∧
    Keygen.instructionAt image 0x1a70 = some (.base (.ADDI .x7 .x7 0x600)) ∧
    Keygen.instructionAt image 0x1a74 = some (.base (.ADD .x7 .x7 .x6)) ∧
    Keygen.instructionAt image 0x1a78 = some (.base (.LBU .x7 .x7 0)) ∧
    Keygen.instructionAt image 0x1a7c = some (.base (.BNE .x7 .x21 0x30)) := by decide

theorem digit_steps (s : MachineState) (pc : s.pc = 0x1a60)
...
theorem digit_pc (s : MachineState) (pc : s.pc = 0x1a60) :
    (digitState s).pc =
      if (s.getByte (0x80600 + s.getMem 0x81030)).zeroExtend 64 ≠
          s.getReg .x21 then 0x1aac else 0x1a80 := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperCaptureDigitAfter67
