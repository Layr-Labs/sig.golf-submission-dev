...
/-! A parent-tree tick leaves earlier WOTS signature words intact.  The
first tick copies one authentication sibling at `witnessBase + 16*level`;
all its other writes, and all later ticks' writes, are at `0x80000` or above. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedTreeTickFrame67
...
private theorem ne_static (a : Word) (low : a.toNat<0x80000)
    (b : Nat) (hb : 0x80000≤b) (small : b<2^64) :
...
theorem first_hashed_before (hash : Hash) (s : MachineState)
  ...
  have answerFrame :
    ...
    apply Signing.hash_answer_frame (ready s) (hash (hashInput (ready s)))
      args.2.2.1 a
    intro i eq
    exact ne_static a low (0x80300+8*i.val) (by omega) (by omega) eq
  exact answerFrame.trans (readyFrame.trans (childFrame.trans
    (pairFrame.trans copyFrame)))

theorem first_tick_before (hash : Hash) (s : MachineState)
  ...
  have ne0 : a≠ptr.getReg .x7 := by
    ...
    rw [targetPtr,BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt (by rcases targetCase with rfl | rfl <;> omega)] at hn
    rcases targetCase with rfl | rfl <;> omega
  have ne8 : a≠ptr.getReg .x7+8 := by
    intro eq
    have hn := congrArg BitVec.toNat eq
    have targetSmall : target<2^64 := by
      rcases targetCase with rfl | rfl <;> omega
    have nextSmall : target+8<2^64 := by
      rcases targetCase with rfl | rfl <;> omega
    have eight : (8:Word).toNat=8 := by decide
    rw [BitVec.toNat_add,targetPtr,BitVec.toNat_ofNat,
      Nat.mod_eq_of_lt targetSmall,eight,
      Nat.mod_eq_of_lt nextSmall] at hn
    rcases targetCase with rfl | rfl <;> omega
  ...
  exact first_hashed_before hash s level witnessBase a levelWord witness
    levelBound witnessBound before
...
theorem inner_hashed_below (hash : Hash) (s : MachineState) (a : Word)
    (low : a.toNat<0x80000) :
...
theorem inner_tick_below (hash : Hash) (s : MachineState)
  ...
  rw [GroupedBalancedSignBottomTreeInnerTickFrame67.tickState,
    GroupedBalancedSignBottomTreeParentControl67.branch_frame,
    advFrame,storedFrame]
  exact inner_hashed_below hash s a low

#print axioms first_tick_before
#print axioms inner_tick_below
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedTreeTickFrame67
