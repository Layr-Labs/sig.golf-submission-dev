...
namespace SigGolfCandidate.Hypertree.GroupedPrivateSeparation
open SigGolf OracleComp OracleSpec GroupedPrivateDerivation SecuritySecretKey
...
/-- A secret key guess must use an actual grouped private-derivation input shape. -/
...
theorem public_hit_implies_secretKeyAt (secretKey : SecretKey) (query : Query)
    (hit : publicSecretKeyHit secretKey (.inr query)) : SecretKeyAt query secretKey :=
  hit.2

theorem prob_public_hit_le_256 (query : Query) :
    Pr[fun secretKey => publicSecretKeyHit secretKey (.inr query) | sampleSecretKey] ≤
      1 / (2 : ENNReal) ^ 256 := by
  exact (probEvent_mono (fun secretKey _ hit =>
    public_hit_implies_secretKeyAt secretKey query hit)).trans
    (prob_secretKeyAt_le_256 query)

def PublicHitTrace (inputs : List Query) (secretKey : SecretKey) : Prop :=
  ∃ query ∈ inputs, publicSecretKeyHit secretKey (.inr query)

theorem prob_public_hit_trace_le_256 (inputs : List Query) :
    Pr[PublicHitTrace inputs | sampleSecretKey] ≤
      inputs.length / (2 : ENNReal) ^ 256 := by
  induction inputs with
  | nil => simp [PublicHitTrace]
  | cons query rest ih =>
      have event : PublicHitTrace (query :: rest) = fun secretKey =>
          publicSecretKeyHit secretKey (.inr query) ∨ PublicHitTrace rest secretKey := by
        funext secretKey
        simp [PublicHitTrace]
      rw [event]
      calc
        _ ≤ Pr[fun secretKey => publicSecretKeyHit secretKey (.inr query) |
              sampleSecretKey] + Pr[PublicHitTrace rest | sampleSecretKey] :=
          probEvent_or_le _ _ _
        _ ≤ 1 / (2 : ENNReal) ^ 256 +
              rest.length / (2 : ENNReal) ^ 256 :=
          add_le_add (prob_public_hit_le_256 query) ih
        _ = _ := by simp [Nat.cast_add, ENNReal.add_div, add_comm]

theorem prob_independent_public_trace_hit_le_256
    (trace : ProbComp (List Query)) (limit : Nat)
    (bounded : ∀ inputs ∈ support trace, inputs.length ≤ limit) :
    Pr[fun result => PublicHitTrace result.1 result.2 | do
      let inputs ← trace
      let secretKey ← sampleSecretKey
      return (inputs, secretKey)] ≤ limit / (2 : ENNReal) ^ 256 := by
  apply probEvent_bind_le_of_forall_le
  intro inputs member
  simp only [← map_eq_pure_bind, probEvent_map, Function.comp_def]
  exact (prob_public_hit_trace_le_256 inputs).trans
    (ENNReal.div_le_div (by exact_mod_cast bounded inputs member) le_rfl)
...
independent private random oracle until a public query guesses the secret key.
The typed slots include bottom seeds, upper paired seeds, and randomizers. -/
...
end SigGolfCandidate.Hypertree.GroupedPrivateSeparation
