...
namespace SigGolfCandidate.Hypertree.VerifyLeafHeaderDirect
...
def Code (image : Image) (p : Word) : Prop :=
  ...
  instructionAt image (p + 48) = some (.base (.ADDI .x28 .x28 2016)) ∧
  ...
  instructionAt image (p + 72) = some (.base (.ADDI .x28 .x28 2024)) ∧
  ...
  instructionAt image (p + 96) = some (.base (.ADDI .x28 .x28 2032)) ∧
  ...
  instructionAt image (p + 120) = some (.base (.ADDI .x28 .x28 2040)) ∧
  instructionAt image (p + 124) = some (.base (.SD .x28 .x11 0)) ∧
  instructionAt image (p + 128) = some (.base (.LUI .x10 128)) ∧
  instructionAt image (p + 132) = some (.base (.ADDI .x10 .x10 2016)) ∧
...
theorem block (image : Image) (p : Word) (code : Code image p)
  ...
  let s13 := execInstrBr s12 (.ADDI .x28 .x28 2016)
  ...
  let s19 := execInstrBr s18 (.ADDI .x28 .x28 2024)
  ...
  let s25 := execInstrBr s24 (.ADDI .x28 .x28 2032)
  ...
  let s31 := execInstrBr s30 (.ADDI .x28 .x28 2040)
  let s32 := execInstrBr s31 (.SD .x28 .x11 0)
  let s33 := execInstrBr s32 (.LUI .x10 128)
  let s34 := execInstrBr s33 (.ADDI .x10 .x10 2016)
  ...
  · rfl
  apply OrdinarySteps.step s12 s13 _ (.base (.ADDI .x28 .x28 2016)) 26
  ...
  · rfl
  apply OrdinarySteps.step s18 s19 _ (.base (.ADDI .x28 .x28 2024)) 20
  ...
  · rfl
  apply OrdinarySteps.step s24 s25 _ (.base (.ADDI .x28 .x28 2032)) 14
  ...
  · rfl
  apply OrdinarySteps.step s30 s31 _ (.base (.ADDI .x28 .x28 2040)) 8
  ...
  · rfl
  apply OrdinarySteps.step s33 s34 _ (.base (.ADDI .x10 .x10 2016)) 5
...
theorem regs (s : MachineState) :
    (state s).getReg .x5 = 1 ∧ (state s).getReg .x10 = 0x807e0 ∧
...
theorem mem (s : MachineState) (a : Word) :
    (state s).getMem a =
      if a = 0x807f8 then s.getMem 0x80418 else
      if a = 0x807f0 then s.getMem 0x80410 else
      if a = 0x807e8 then s.getMem 0x80408 else
      if a = 0x807e0 then 3 + (s.getMem 0x80400 <<< 8) + (s.getMem 0x80428 <<< 16) else s.getMem a := by
  simp [state,execInstrBr,signExtend12,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
...
theorem frame (s : MachineState) (a : Word)
    (outside : ∀ i : Fin 4, a ≠ Signing.wordAddress 0x807e0 i.val) :
    (state s).getMem a = s.getMem a := by
  have h0 : a ≠ (0x807e0:Word) := outside 0
  have h1 : a ≠ (0x807e8:Word) := outside 1
  have h2 : a ≠ (0x807f0:Word) := outside 2
  have h3 : a ≠ (0x807f8:Word) := outside 3
...
theorem words (s : MachineState) (level tree leaf : Nat) (values : Reference.Chain → Reference.Digest)
    ...
    (hvalues : ∀ i : Fin 92, s.getMem (Signing.wordAddress 0x80800 i.val) = endpointWord values i) :
    ∀ i : Fin 96, (state s).getMem (Signing.wordAddress 0x807e0 i.val) =
  ...
  · obtain ⟨i,hi⟩ := i
    ...
    · exact h1
    · exact h2
  · have outside : ∀ j : Fin 4, Signing.wordAddress 0x807e0 i.val ≠ Signing.wordAddress 0x807e0 j.val := by
      intro j
      exact Signing.wordAddress_injective 0x807e0 96 (by decide) i.val j.val i.isLt (by have := j.isLt; omega) (by have := j.isLt; omega)
    rw [frame s _ outside]
    have index : i.val/1-4 < 92 := by have := i.isLt; omega
    have addr : Signing.wordAddress 0x807e0 i.val = Signing.wordAddress 0x80800 (i.val-4) := by
...
end SigGolfCandidate.Hypertree.VerifyLeafHeaderDirect
