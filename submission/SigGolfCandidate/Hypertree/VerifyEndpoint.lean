...
def CodeAt (image : Image) : Prop :=
  ...
  instructionAt image 0x1664 = some (.base (.BNE .x6 .x20 (-460))) ∧
  instructionAt image 0x1668 = some (.base (.JAL .x0 40))
...
def core (s : MachineState) : MachineState :=
  ...
  execInstrBr s (.SD .x28 .x6 1072)

/-- Non-final chains branch straight back to the chain body; only the last chain takes the exit jump. -/
def stateAt (s : MachineState) : MachineState :=
  let branched := execInstrBr (core s) (.BNE .x6 .x20 (-460))
  if s.getMem 0x80430 + 1 = 46 then
    execInstrBr branched (.JAL .x0 40)
  else branched

theorem state_pc (s : MachineState) (pc : s.pc = 0x1644)
    (counterReg : s.getReg .x6 = s.getMem 0x80430)
    (limit : s.getReg .x20 = 46) :
    (stateAt s).pc = if s.getMem 0x80430 + 1 = 46 then 0x1690 else 0x1498 := by
  simp [stateAt, core, execInstrBr, pc, counterReg, limit, signExtend12, signExtend13, signExtend21,
    MachineState.getReg_setReg_eq, MachineState.getReg_setReg_ne]
  split_ifs <;> decide

theorem state_regs (s : MachineState) (baseReg : s.getReg .x28 = 0x80000) :
    (stateAt s).getReg .x28 = 0x80000 ∧
    (stateAt s).getReg .x6 = s.getReg .x6 + 1 ∧
    (stateAt s).getReg .x20 = s.getReg .x20 ∧
...
theorem state_mem (s : MachineState)
  ...
  simp [stateAt, core, KeygenEndpoint.stateAt, execInstrBr, signExtend12,
    destination, counterReg, baseReg, BitVec.add_assoc,
    MachineState.getReg_setReg_eq, MachineState.getReg_setReg_ne]
...
theorem state_sticky (s : MachineState) :
    ...
    (stateAt s).getReg .x11 = s.getReg .x11 ∧
    (stateAt s).getReg .x20 = s.getReg .x20 := by
  simp [stateAt, core, execInstrBr, MachineState.getReg_setReg_ne]
...
theorem state_post (s : MachineState) (chain : Reference.Chain)
  ...
  · rw [state_pc s pc counterReg limit]
    simp only [eq]
  · rw [state_mem s baseReg limit destination counterReg, KeygenEndpoint.memAt, if_pos rfl, inc]
  · intro i
    rw [state_mem s baseReg limit destination counterReg, KeygenEndpoint.memAt, if_neg (neCounter i), addr8, addr]
    fin_cases i
  ...
  · intro a hc outside
    ...
    rw [state_mem s baseReg limit destination counterReg, KeygenEndpoint.memAt, if_neg hc, addr8, if_neg h1, addr, if_neg h0]


theorem ordinarySteps_append {image : Image} {a b c : MachineState} {m n : Nat}
    (first : OrdinarySteps image a m b) (second : OrdinarySteps image b n c) :
    OrdinarySteps image a (n + m) c := by
  induction first with
  | refl => simpa using second
  | step state next _ instruction steps hf hs _ ih =>
    simpa [Nat.add_assoc] using OrdinarySteps.step state next c instruction (n + steps) hf hs (ih second)

theorem final_iff (s : MachineState) (chain : Reference.Chain)
    (counter : s.getMem 0x80430 = BitVec.ofNat 64 chain.val) :
    s.getMem 0x80430 + 1 = 46 ↔ chain.val+1 = 46 := by
  have inc : s.getMem 0x80430 + 1 = BitVec.ofNat 64 (chain.val+1) := by
    rw [counter, BitVec.ofNat_add]; rfl
  rw [inc]
  constructor
...
/-- The x12-relative endpoint reuses the live chain register in twelve instructions on the last chain and eleven otherwise. -/
theorem block (image : Image) (code : CodeAt image) (s : MachineState)
    ...
    (limit : s.getReg .x20 = 46)
    (destination : s.getReg .x12 = 0x80020) :
    OrdinarySteps image s (if chain.val+1 = 46 then 10 else 9) (stateAt s) := by
  obtain ⟨c2,c3,c4,c5,c6,c7,c8,c9,c11,c12⟩ := code
  have counterReg : s.getReg .x6 = s.getMem 0x80430 :=
    chainReg.trans counter.symm
  ...
  let s11 := s10
  let s12 := execInstrBr s11 (.BNE .x6 .x20 (-460))
  have branchPC : s12.pc =
      if s.getMem 0x80430 + 1 = 46 then 0x1668 else 0x1498 := by
    simp [s12,s11,s10,s9,s8,s7,s6,s5,s4,s3,s2,s1,
      execInstrBr,pc,counterReg,limit,signExtend12,signExtend13,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  have coreEq : s11 = core s := rfl
  have pre : OrdinarySteps image s 9 s12 := by
    apply OrdinarySteps.step s s3 _ (.base (.SLLI .x7 .x6 4)) 8
    · have hp : s.pc = 0x1644 := pc
      simpa only [fetch_at,hp] using c2
    · rfl
    apply OrdinarySteps.step s3 s4 _ (.base (.ADD .x7 .x7 .x12)) 7
    · have hp : s3.pc = 0x1648 := by simp [s1,s2,s3,execInstrBr,pc]
      simpa only [fetch_at,hp] using c3
    · rfl
    apply OrdinarySteps.step s4 s5 _ (.base (.LD .x18 .x28 32)) 6
    ...
    apply OrdinarySteps.step s5 s6 _ (.base (.LD .x19 .x28 40)) 5
    ...
    · simp [s1,s2,s3,s4,s5,s6,ordinaryStep,memoryArgumentsValid,execInstrBr,
        signExtend12,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne,baseReg,
        accessValid,rangeValid,MEMORY_BYTES]
    apply OrdinarySteps.step s6 s7 _ (.base (.SD .x7 .x18 2016)) 4
    ...
    apply OrdinarySteps.step s7 s8 _ (.base (.SD .x7 .x19 2024)) 3
    ...
    apply OrdinarySteps.step s8 s9 _ (.base (.ADDI .x6 .x6 1)) 2
    · have hp : s8.pc = 0x165c := by simp [s1,s2,s3,s4,s5,s6,s7,s8,execInstrBr,pc]
      simpa only [fetch_at,hp] using c8
    · rfl
    apply OrdinarySteps.step s9 s10 _ (.base (.SD .x28 .x6 1072)) 1
    ...
    · simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,ordinaryStep,memoryArgumentsValid,
        execInstrBr,signExtend12,MachineState.getReg_setReg_eq,baseReg,
        MachineState.getReg_setReg_ne,accessValid,rangeValid,MEMORY_BYTES]
    apply OrdinarySteps.step s11 s12 _ (.base (.BNE .x6 .x20 (-460))) 0
    · have hp : s11.pc = 0x1664 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,execInstrBr,pc]
      simpa only [fetch_at,hp] using c11
    · rfl
    exact OrdinarySteps.refl _

  have final := final_iff s chain counter
  by_cases h : s.getMem (0x80430 : Word) + (1 : Word) = (46 : Word)
  · have hb : s.getMem 525360#64 + 1#64 = 46#64 := h
    have jump : OrdinarySteps image s12 1 (stateAt s) := by
      apply OrdinarySteps.step s12 (stateAt s) _ (.base (.JAL .x0 40)) 0
      · have hp : s12.pc = 0x1668 := by rw [branchPC]; simp [hb]
        simpa only [fetch_at,hp] using c12
      · unfold stateAt
        rw [if_pos h]
        change ordinaryStep s12 (.base (.JAL .x0 40)) =
          some (execInstrBr s12 (.JAL .x0 40))
        rfl
      exact OrdinarySteps.refl _
    rw [if_pos (final.mp h)]
    exact ordinarySteps_append pre jump
  · have same : stateAt s = s12 := by
      unfold stateAt
      rw [if_neg h]
      rfl
    rw [if_neg (fun e => h (final.mpr e)), same]
    exact pre
...
theorem endpoint_short_header_word_carry (s : MachineState) (level tree leaf : Nat)
  ...
  rcases carry with ⟨oldChain, oldStep, hc, hs, header, index, service, destination, seven,
    source, length, limit⟩
  obtain ⟨r5, r12, r31, r10, r11, r20⟩ := EndpointShort.state_sticky s
  refine ⟨oldChain, oldStep, hc, hs, ?_, ?_, r5.trans service,
    ...
    r20.trans limit⟩
  · rw [EndpointShort.state_mem s baseReg limit destination counterReg]
...
theorem store_endpoint_with_word_carry (s : MachineState) (level tree leaf : Nat)
    ...
    (valueWords : ∀ i : Fin 2,
      s.getMem (wordAddress 0x80020 i.val) = value.extractLsb' (64*i.val) 64)
    (carry : Hoist.HeaderWordCarry s level tree leaf) :
    ∃ final, OrdinarySteps verify s (if chain.val+1 = 46 then 10 else 9) final ∧
  ...
  obtain ⟨finalPC, finalCounter, endpoints, ra, sp, frame⟩ :=
    EndpointShort.state_post s chain value pc counter chainReg baseReg limit destination valueWords
  have nextEntry : ChainEntry (EndpointShort.stateAt s) (chain.val+1) := by
    ...
    · right; right
      refine ⟨(EndpointShort.state_regs s baseReg).1, ?_⟩
      rw [(EndpointShort.state_regs s baseReg).2.1, chainReg, BitVec.ofNat_add]
      rfl
  exact ⟨EndpointShort.stateAt s,
    EndpointShort.block verify EndpointShort.verify_code s chain pc counter chainReg baseReg limit destination,
    nextEntry, finalCounter, endpoints, ra, sp, frame,
    endpoint_short_header_word_carry s level tree leaf chain counter chainReg baseReg carry⟩
