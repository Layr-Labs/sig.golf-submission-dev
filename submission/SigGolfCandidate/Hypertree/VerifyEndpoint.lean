...
def CodeAt (image : Image) : Prop :=
  ...
  instructionAt image 0x1648 = some (.base (.LUI .x10 129)) ∧
  instructionAt image 0x164c = some (.base (.ADDI .x10 .x10 2048)) ∧
  instructionAt image 0x1650 = some (.base (.ADD .x7 .x7 .x10)) ∧
  instructionAt image 0x1654 = some (.base (.LD .x10 .x28 32)) ∧
  instructionAt image 0x1658 = some (.base (.LD .x11 .x28 40)) ∧
  instructionAt image 0x165c = some (.base (.SD .x7 .x10 0)) ∧
  instructionAt image 0x1660 = some (.base (.SD .x7 .x11 8)) ∧
  instructionAt image 0x1664 = some (.base (.ADDI .x6 .x6 1)) ∧
  instructionAt image 0x1668 = some (.base (.SD .x28 .x6 1072)) ∧
...
theorem state_mem (s : MachineState) (a : Word) :
...
theorem state_post (s : MachineState) (chain : Reference.Chain)
  ...
  · rw [state_pc s pc]
    simp only [eq]
  · rw [state_mem, KeygenEndpoint.memAt, if_pos rfl, inc]
  · intro i
    rw [state_mem, KeygenEndpoint.memAt, if_neg (neCounter i), addr8, addr]
    fin_cases i
  ...
  · intro a hc outside
    ...
    rw [state_mem, KeygenEndpoint.memAt, if_neg hc, addr8, if_neg h1, addr, if_neg h0]


/-- The shortened verifier endpoint uses exactly fifteen ordinary instructions. -/
theorem block (image : Image) (code : CodeAt image) (s : MachineState)
    (chain : Reference.Chain)
    (pc : s.pc = 0x163c)
    (counter : s.getMem 0x80430 = BitVec.ofNat 64 chain.val) :
    OrdinarySteps image s 15 (stateAt s) := by
  obtain ⟨c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15⟩ := code
  ...
  let s4 := execInstrBr s3 (.LUI .x10 129)
  let s5 := execInstrBr s4 (.ADDI .x10 .x10 2048)
  let s6 := execInstrBr s5 (.ADD .x7 .x7 .x10)
  let s7 := execInstrBr s6 (.LD .x10 .x28 32)
  let s8 := execInstrBr s7 (.LD .x11 .x28 40)
  let s9 := execInstrBr s8 (.SD .x7 .x10 0)
  let s10 := execInstrBr s9 (.SD .x7 .x11 8)
  let s11 := execInstrBr s10 (.ADDI .x6 .x6 1)
  let s12 := execInstrBr s11 (.SD .x28 .x6 1072)
  let s13 := execInstrBr s12 (.ADDI .x7 .x0 46)
  let s14 := execInstrBr s13 (.BNE .x6 .x7 8)
  have branchPC : s14.pc =
      if s.getMem 0x80430 + 1 = 46 then 0x1674 else 0x1678 := by
    simp [s14,s13,s12,s11,s10,s9,s8,s7,s6,s5,s4,s3,s2,s1,
      execInstrBr,pc,signExtend12,signExtend13,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  have coreEq : s13 = core s := rfl
  apply OrdinarySteps.step s s1 _ (.base (.LUI .x28 128)) 14
  · simpa only [fetch_at,pc] using c0
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.LD .x6 .x28 1072)) 13
  · have hp : s1.pc = 0x1640 := by
      simp [s1,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  ...
  apply OrdinarySteps.step s2 s3 _ (.base (.SLLI .x7 .x6 4)) 12
  · have hp : s2.pc = 0x1644 := by
      simp [s1,s2,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c2
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.LUI .x10 129)) 11
  · have hp : s3.pc = 0x1648 := by
      simp [s1,s2,s3,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c3
  · rfl
  apply OrdinarySteps.step s4 s5 _ (.base (.ADDI .x10 .x10 2048)) 10
  · have hp : s4.pc = 0x164c := by
      simp [s1,s2,s3,s4,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c4
  · rfl
  apply OrdinarySteps.step s5 s6 _ (.base (.ADD .x7 .x7 .x10)) 9
  · have hp : s5.pc = 0x1650 := by
      simp [s1,s2,s3,s4,s5,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c5
  · rfl
  apply OrdinarySteps.step s6 s7 _ (.base (.LD .x10 .x28 32)) 8
  · have hp : s6.pc = 0x1654 := by
      simp [s1,s2,s3,s4,s5,s6,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c6
  · simp [s1,s2,s3,s4,s5,s6,s7,ordinaryStep,memoryArgumentsValid,execInstrBr,signExtend12,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne,
      accessValid,rangeValid,MEMORY_BYTES]
  apply OrdinarySteps.step s7 s8 _ (.base (.LD .x11 .x28 40)) 7
  · have hp : s7.pc = 0x1658 := by
      simp [s1,s2,s3,s4,s5,s6,s7,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c7
  · simp [s1,s2,s3,s4,s5,s6,s7,s8,ordinaryStep,memoryArgumentsValid,execInstrBr,signExtend12,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne,
      accessValid,rangeValid,MEMORY_BYTES]
  apply OrdinarySteps.step s8 s9 _ (.base (.SD .x7 .x10 0)) 6
  · have hp : s8.pc = 0x165c := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  ...
  · simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,ordinaryStep,memoryArgumentsValid,execInstrBr,signExtend12,
    ...
    change accessValid (KeygenEndpoint.address s) 8 = true
    exact safe.1
  apply OrdinarySteps.step s9 s10 _ (.base (.SD .x7 .x11 8)) 5
  · have hp : s9.pc = 0x1660 := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  ...
  · simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,ordinaryStep,memoryArgumentsValid,execInstrBr,signExtend12,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne,
      MachineState.getReg_setReg_ne]
    change accessValid (KeygenEndpoint.address s + 8) 8 = true
    exact safe.2
  apply OrdinarySteps.step s10 s11 _ (.base (.ADDI .x6 .x6 1)) 4
  · have hp : s10.pc = 0x1664 := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c10
  · rfl
  apply OrdinarySteps.step s11 s12 _ (.base (.SD .x28 .x6 1072)) 3
  · have hp : s11.pc = 0x1668 := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
  ...
  · simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,ordinaryStep,memoryArgumentsValid,execInstrBr,signExtend12,
      MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne,
      accessValid,rangeValid,MEMORY_BYTES]
  apply OrdinarySteps.step s12 s13 _ (.base (.ADDI .x7 .x0 46)) 2
  · have hp : s12.pc = 0x166c := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c12
  · rfl
  apply OrdinarySteps.step s13 s14 _ (.base (.BNE .x6 .x7 8)) 1
  · have hp : s13.pc = 0x1670 := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,execInstrBr,pc,BitVec.add_assoc,signExtend12,signExtend13,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]
    simpa only [fetch_at,hp] using c13
  · rfl
  by_cases h : s.getMem (0x80430 : Word) + (1 : Word) = (46 : Word)
  · have hb : s.getMem 525360#64 + 1#64 = 46#64 := h
    apply OrdinarySteps.step s14 (stateAt s) _ (.base (.JAL .x0 28)) 0
    · have hp : s14.pc = 0x1674 := by rw [branchPC]; simp [hb]
      simpa only [fetch_at,hp] using c14
    · unfold stateAt
      rw [if_pos h]
      change ordinaryStep s14 (.base (.JAL .x0 28)) =
        some (execInstrBr s14 (.JAL .x0 28))
      rfl
