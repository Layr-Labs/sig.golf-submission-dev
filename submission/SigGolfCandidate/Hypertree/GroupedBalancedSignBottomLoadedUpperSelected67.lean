import SigGolfCandidate.Hypertree.GroupedBalancedSignBottomSelectedEntry67
...
/-! Inlined from SigGolfCandidate.Hypertree.GroupedBalancedSignBottomSelectedReturn67; its only importer was SigGolfCandidate.Hypertree.GroupedBalancedSignBottomLoadedUpperSelected67. -/
section
/-! Return from the bottom tree with all ten selected siblings still in the wire. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignBottomSelectedReturn67
...
theorem bottom_tree_returned_selected (hash : Hash) (secretKey : SecretKey)
    ...
    ∃ returned : MachineState,
      ...
      returned.getMem 0x81000=10 ∧
      (∀ level, level<10 → ∀ i : Fin 2,
        returned.getMem (pathSlot level i) =
          (siblingNode hash secretKey base selectedIndex level).extractLsb'
            (64*i.val) 64) := by
  have selectedSmall : (s.getMem 0x810e8).toNat<1024 := by
    rw [selectedWord]
    exact selectedBound
  obtain ⟨start,entryTrace,params,startInv,startSelected,_,_,entryFrame⟩ :=
    GroupedBalancedSignBottomSelectedEntry67.start_selected hash secretKey
      base selectedIndex s pc sp aligned bounded count height maxLevel witnessBase
      selectedWord selectedBound scratch leafWords
  obtain ⟨pathFinal,_,pathTrace,pathPc,_,_,_,pathWords⟩ :=
    GroupedBalancedSignBottomSelectedLevels67.ten_levels_selected
      hash secretKey base 512 0x83000 0x88000 (base/2) selectedIndex
      start params startInv startSelected
  have pathWhole : Trace hash image s 87096 94257 1023 1023 pathFinal := by
    simpa [image] using entryTrace.trans pathTrace
  obtain ⟨stackFinal,stackTrace,_,source,rootWords,stack,saved,stackFrame,
    treeWord⟩ :=
  ...
  have same := Trace.deterministic pathWhole stackTrace
  have finalSp : pathFinal.getReg .x2 = s.getReg .x2-16 := by
    rw [same]
    exact stack
  have finalLink : pathFinal.getMem (pathFinal.getReg .x2)=0x14f0 := by
    rw [finalSp,same]
    exact saved
  have valid : accessValid (pathFinal.getReg .x2) 8 = true := by
    rw [finalSp]
    rcases sp with h | h <;> rw [h] <;> decide
  let returned := Keygen.returnState pathFinal
  have retTrace := GroupedBalancedSignBottomTreeReturn67.return_trace
    hash pathFinal pathPc valid
  refine ⟨returned,?_,GroupedBalancedSignBottomTreeReturn67.return_pc
    pathFinal finalLink,?_,?_,?_,?_,?_,?_⟩
  · simpa [returned,image] using pathWhole.trans retTrace
  · rw [GroupedBalancedSignBottomTreeReturn67.return_sp,finalSp]
    rcases sp with h | h <;> rw [h] <;> decide
  · rw [GroupedBalancedSignBottomTreeReturn67.return_mem,same]
    exact source
  · intro i
    rw [GroupedBalancedSignBottomTreeReturn67.return_mem,same]
    exact rootWords i
  · intro a safe neSlot
    rw [GroupedBalancedSignBottomTreeReturn67.return_mem,same]
    exact stackFrame a safe neSlot
  · rw [GroupedBalancedSignBottomTreeReturn67.return_mem,same]
    exact treeWord
  · intro level hlevel i
    rw [GroupedBalancedSignBottomTreeReturn67.return_mem]
    exact pathWords level hlevel i

#print axioms bottom_tree_returned_selected
end SigGolfCandidate.Hypertree.GroupedBalancedSignBottomSelectedReturn67

end

/-! The official loaded signer returns from the bottom tree and enters upper signing. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignBottomLoadedUpperSelected67
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64
open SigGolfCandidate.Hypertree
...
theorem loaded_bottom_to_upper_selected (hash : Hash) (secretKey : SecretKey)
  ...
  have lowWord : finish.getMem 0x81090 = index.extractLsb' 0 64 := by
    have first := indexWords (0 : Fin 3)
    change finish.getMem 0x81090 =
      (BitVec.ofNat 192 index.toNat).extractLsb' 0 64 at first
    rw [first]
    exact GroupedBalancedSignBottomAddress67.widenedSlice index 0
  have selectedNat : (finish.getMem 0x810e8).toNat =
  ...
  obtain ⟨returned,tree,returnedPc,returnedSp,source,rootWords,frame,
    returnedTree,returnedSiblings⟩ :=
    GroupedBalancedSignBottomSelectedReturn67.bottom_tree_returned_selected
      hash secretKey base ((index.extractLsb' 0 10).toNat) finish finishPc
      (Or.inr sp) aligned bounded count height maxLevel witnessBase
      selectedNat (index.extractLsb' 0 10).isLt scratch leafWords'
  ...
  refine ⟨initial,upper,n+87099+366,c+94260+366,loaded,?_,upperPc,?_,
    ...
    (upperFrame 0x81000 (by simp [GroupedBalancedSignBottomToUpper67.Outside])).trans
      returnedTree,?_,?_,by omega,by omega,?_,?_,?_⟩
  ...
  · intro level hlevel i
    have small :
        (GroupedBalancedSignBottomSelectedLevels67.pathSlot level i).toNat <
          0x80500 := by
      unfold GroupedBalancedSignBottomSelectedLevels67.pathSlot
      have hi := i.isLt
      rw [BitVec.toNat_ofNat,Nat.mod_eq_of_lt (by omega)]
      omega
    have ne (b : Word) (high : 0x80500 ≤ b.toNat) :
        GroupedBalancedSignBottomSelectedLevels67.pathSlot level i ≠ b := by
      intro eq
      rw [eq] at small
      omega
    have outside : GroupedBalancedSignBottomToUpper67.Outside
      ...
      ⟨ne _ (by decide),ne _ (by decide),ne _ (by decide),
        ne _ (by decide),ne _ (by decide),ne _ (by decide),
        ne _ (by decide),ne _ (by decide),ne _ (by decide)⟩
    exact (upperFrame _ outside).trans (returnedSiblings level hlevel i)

#print axioms loaded_bottom_to_upper_selected
end SigGolfCandidate.Hypertree.GroupedBalancedSignBottomLoadedUpperSelected67
