import SigGolfCandidate.Hypertree.GroupedBalancedVerifyH2InputCopy67

/-! Exact H2 query setup after loading the first witness sibling. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedVerifyH2Entry67
...
private theorem setup_code :
    Keygen.instructionAt image 0x11b0 = some (.base (.ADDI .x10 .x0 2)) ∧
    Keygen.instructionAt image 0x11b4 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x11b8 = some (.base (.ADDI .x28 .x28 0)) ∧
    Keygen.instructionAt image 0x11bc = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x11c0 = some (.base (.SLLI .x11 .x11 8)) ∧
    Keygen.instructionAt image 0x11c4 = some (.base (.ADD .x10 .x10 .x11)) ∧
    Keygen.instructionAt image 0x11c8 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x11cc = some (.base (.ADDI .x28 .x28 0)) ∧
    Keygen.instructionAt image 0x11d0 = some (.base (.SD .x28 .x10 0)) ∧
    Keygen.instructionAt image 0x11d4 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x11d8 = some (.base (.ADDI .x28 .x28 8)) ∧
    Keygen.instructionAt image 0x11dc = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x11e0 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x11e4 = some (.base (.ADDI .x28 .x28 8)) ∧
    Keygen.instructionAt image 0x11e8 = some (.base (.SD .x28 .x11 0)) ∧
    Keygen.instructionAt image 0x11ec = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x11f0 = some (.base (.ADDI .x28 .x28 16)) ∧
    Keygen.instructionAt image 0x11f4 = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x11f8 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x11fc = some (.base (.ADDI .x28 .x28 16)) ∧
    Keygen.instructionAt image 0x1200 = some (.base (.SD .x28 .x11 0)) ∧
    Keygen.instructionAt image 0x1204 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1208 = some (.base (.ADDI .x28 .x28 24)) ∧
    Keygen.instructionAt image 0x120c = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1210 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x1214 = some (.base (.ADDI .x28 .x28 24)) ∧
    Keygen.instructionAt image 0x1218 = some (.base (.SD .x28 .x11 0)) ∧
    Keygen.instructionAt image 0x121c = some (.base (.LUI .x10 0x80)) ∧
    Keygen.instructionAt image 0x1220 = some (.base (.ADDI .x10 .x10 0)) ∧
    Keygen.instructionAt image 0x1224 = some (.base (.ADDI .x11 .x0 384)) ∧
    Keygen.instructionAt image 0x1228 = some (.base (.LUI .x12 0x80)) ∧
    Keygen.instructionAt image 0x122c = some (.base (.ADDI .x12 .x12 0x300)) ∧
    Keygen.instructionAt image 0x1230 = some (.base (.ADDI .x5 .x0 1))
    := by
  unfold image GroupedBalancedVerifyImage67Fast2Byte.image
    GroupedBalancedVerifyImage67Fast2Byte.code
  decide

theorem setup_steps (s : MachineState) (pc : s.pc = 0x11b0) :
    OrdinarySteps image s 33 (setupState s) := by
  ...
  obtain ⟨c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,c21,c22,c23,c24,c25,c26,c27,c28,c29,c30,c31,c32⟩ := setup_code
  apply OrdinarySteps.step s s1 _ (.base (.ADDI .x10 .x0 2)) 32
  · simpa only [Keygen.fetch_at,pc] using c0
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.LUI .x28 0x81)) 31
  · have hp : s1.pc = 0x11b4 := by simp [s1,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c1
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.ADDI .x28 .x28 0)) 30
  · have hp : s2.pc = 0x11b8 := by simp [s1,s2,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c2
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.LD .x11 .x28 0)) 29
  · have hp : s3.pc = 0x11bc := by simp [s1,s2,s3,execInstrBr,pc]
  ...
  · have hp : s9.pc = 0x11d4 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c9
  · rfl
  apply OrdinarySteps.step s10 s11 _ (.base (.ADDI .x28 .x28 8)) 22
  · have hp : s10.pc = 0x11d8 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c10
  · rfl
  apply OrdinarySteps.step s11 s12 _ (.base (.LD .x11 .x28 0)) 21
  · have hp : s11.pc = 0x11dc := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,execInstrBr,pc]
  ...
  · have hp : s12.pc = 0x11e0 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c12
  · rfl
  apply OrdinarySteps.step s13 s14 _ (.base (.ADDI .x28 .x28 8)) 19
  · have hp : s13.pc = 0x11e4 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c13
  · rfl
  apply OrdinarySteps.step s14 s15 _ (.base (.SD .x28 .x11 0)) 18
  · have hp : s14.pc = 0x11e8 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,execInstrBr,pc]
  ...
  · have hp : s15.pc = 0x11ec := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c15
  · rfl
  apply OrdinarySteps.step s16 s17 _ (.base (.ADDI .x28 .x28 16)) 16
  · have hp : s16.pc = 0x11f0 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c16
  · rfl
  apply OrdinarySteps.step s17 s18 _ (.base (.LD .x11 .x28 0)) 15
  · have hp : s17.pc = 0x11f4 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,execInstrBr,pc]
  ...
  · have hp : s18.pc = 0x11f8 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c18
  · rfl
  apply OrdinarySteps.step s19 s20 _ (.base (.ADDI .x28 .x28 16)) 13
  · have hp : s19.pc = 0x11fc := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c19
  · rfl
  apply OrdinarySteps.step s20 s21 _ (.base (.SD .x28 .x11 0)) 12
  · have hp : s20.pc = 0x1200 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,execInstrBr,pc]
  ...
  · have hp : s21.pc = 0x1204 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c21
  · rfl
  apply OrdinarySteps.step s22 s23 _ (.base (.ADDI .x28 .x28 24)) 10
  · have hp : s22.pc = 0x1208 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c22
  · rfl
  apply OrdinarySteps.step s23 s24 _ (.base (.LD .x11 .x28 0)) 9
  · have hp : s23.pc = 0x120c := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,execInstrBr,pc]
  ...
  · have hp : s24.pc = 0x1210 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c24
  · rfl
  apply OrdinarySteps.step s25 s26 _ (.base (.ADDI .x28 .x28 24)) 7
  · have hp : s25.pc = 0x1214 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c25
  · rfl
  apply OrdinarySteps.step s26 s27 _ (.base (.SD .x28 .x11 0)) 6
  · have hp : s26.pc = 0x1218 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,execInstrBr,pc]
  ...
  · have hp : s27.pc = 0x121c := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c27
  · rfl
  apply OrdinarySteps.step s28 s29 _ (.base (.ADDI .x10 .x10 0)) 4
  · have hp : s28.pc = 0x1220 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c28
  · rfl
  apply OrdinarySteps.step s29 s30 _ (.base (.ADDI .x11 .x0 384)) 3
  · have hp : s29.pc = 0x1224 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,s29,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c29
  · rfl
  apply OrdinarySteps.step s30 s31 _ (.base (.LUI .x12 0x80)) 2
  · have hp : s30.pc = 0x1228 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,s29,s30,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c30
  · rfl
  apply OrdinarySteps.step s31 s32 _ (.base (.ADDI .x12 .x12 0x300)) 1
  · have hp : s31.pc = 0x122c := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,s29,s30,s31,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c31
  · rfl
  apply OrdinarySteps.step s32 (setupState s) _ (.base (.ADDI .x5 .x0 1)) 0
  · have hp : s32.pc = 0x1230 := by simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,s29,s30,s31,s32,execInstrBr,pc]
    simpa only [Keygen.fetch_at,hp] using c32
  · rfl
  exact OrdinarySteps.refl _

theorem setup_pc (s : MachineState) (pc : s.pc = 0x11b0) :
    (setupState s).pc = 0x1234 := by
  simp [setupState,execInstrBr,pc]

theorem setup_regs (s : MachineState) :
    (setupState s).getReg .x5 = 1 ∧
    (setupState s).getReg .x10 = 0x80000 ∧
    (setupState s).getReg .x11 = 384 ∧
...
end SigGolfCandidate.Hypertree.GroupedBalancedVerifyH2Entry67
