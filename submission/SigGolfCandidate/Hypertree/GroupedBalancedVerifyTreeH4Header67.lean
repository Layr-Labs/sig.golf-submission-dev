...
/-! Exact 33-instruction first H4 query header in the direct verifier. -/

namespace SigGolfCandidate.Hypertree.GroupedBalancedVerifyTreeH4Header67
...
def HeaderCode (image : Image) : Prop :=
  Keygen.instructionAt image 0x13e4 = some (.base (.ADDI .x10 .x0 4)) ∧
  Keygen.instructionAt image 0x13e8 = some (.base (.LUI .x28 0x81)) ∧
  Keygen.instructionAt image 0x13ec = some (.base (.ADDI .x28 .x28 0)) ∧
  Keygen.instructionAt image 0x13f0 = some (.base (.LD .x11 .x28 0)) ∧
  Keygen.instructionAt image 0x13f4 = some (.base (.SLLI .x11 .x11 8)) ∧
  Keygen.instructionAt image 0x13f8 = some (.base (.ADD .x10 .x10 .x11)) ∧
  Keygen.instructionAt image 0x13fc = some (.base (.LUI .x28 0x80)) ∧
  Keygen.instructionAt image 0x1400 = some (.base (.ADDI .x28 .x28 0)) ∧
  Keygen.instructionAt image 0x1404 = some (.base (.SD .x28 .x10 0)) ∧
  Keygen.instructionAt image 0x1408 = some (.base (.LUI .x28 0x81)) ∧
  Keygen.instructionAt image 0x140c = some (.base (.ADDI .x28 .x28 8)) ∧
  Keygen.instructionAt image 0x1410 = some (.base (.LD .x11 .x28 0)) ∧
  Keygen.instructionAt image 0x1414 = some (.base (.LUI .x28 0x80)) ∧
  Keygen.instructionAt image 0x1418 = some (.base (.ADDI .x28 .x28 8)) ∧
  Keygen.instructionAt image 0x141c = some (.base (.SD .x28 .x11 0)) ∧
  Keygen.instructionAt image 0x1420 = some (.base (.LUI .x28 0x81)) ∧
  Keygen.instructionAt image 0x1424 = some (.base (.ADDI .x28 .x28 16)) ∧
  Keygen.instructionAt image 0x1428 = some (.base (.LD .x11 .x28 0)) ∧
  Keygen.instructionAt image 0x142c = some (.base (.LUI .x28 0x80)) ∧
  Keygen.instructionAt image 0x1430 = some (.base (.ADDI .x28 .x28 16)) ∧
  Keygen.instructionAt image 0x1434 = some (.base (.SD .x28 .x11 0)) ∧
  Keygen.instructionAt image 0x1438 = some (.base (.LUI .x28 0x81)) ∧
  Keygen.instructionAt image 0x143c = some (.base (.ADDI .x28 .x28 24)) ∧
  Keygen.instructionAt image 0x1440 = some (.base (.LD .x11 .x28 0)) ∧
  Keygen.instructionAt image 0x1444 = some (.base (.LUI .x28 0x80)) ∧
  Keygen.instructionAt image 0x1448 = some (.base (.ADDI .x28 .x28 24)) ∧
  Keygen.instructionAt image 0x144c = some (.base (.SD .x28 .x11 0)) ∧
  Keygen.instructionAt image 0x1450 = some (.base (.LUI .x10 0x80)) ∧
  Keygen.instructionAt image 0x1454 = some (.base (.ADDI .x10 .x10 0)) ∧
  Keygen.instructionAt image 0x1458 = some (.base (.ADDI .x11 .x0 512)) ∧
  Keygen.instructionAt image 0x145c = some (.base (.LUI .x12 0x80)) ∧
  Keygen.instructionAt image 0x1460 = some (.base (.ADDI .x12 .x12 0x300)) ∧
  Keygen.instructionAt image 0x1464 = some (.base (.ADDI .x5 .x0 1))
...
theorem header_block (s : MachineState) (pc : s.pc = 0x13e4) :
...
theorem header_pc (s : MachineState) (pc : s.pc = 0x13e4) :
    (headerState s).pc = 0x1468 := by
...
theorem loaded_header (hash : Hash) (input : Input program.sizes .verify) :
    ∃ initial n final,
      initialState program .verify input = some initial ∧
      (n = 339 ∨ n = 340) ∧
      Trace hash image initial n (n+22) 2 3 final ∧
      final.pc = 0x1468 ∧ final.getMem 0x81048 = 0x2c730 := by
  ...
  let final := headerState before
  have headerRun := header_block before beforePC
  refine ⟨initial,n+33,final,loaded,?_,?_,header_pc before beforePC,?_⟩
  · rcases ncases with h | h <;> simp [h]
  · have run := beforeRun.trans headerRun.trace
    simpa [image, final, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using run
  · exact (header_pointer before).trans pointer

#print axioms header_block
#print axioms loaded_header
end SigGolfCandidate.Hypertree.GroupedBalancedVerifyTreeH4Header67
