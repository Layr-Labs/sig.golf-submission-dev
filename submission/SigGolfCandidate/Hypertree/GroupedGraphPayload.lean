...
base, full leaf address, and one of 26 paired chain seeds. All public query
addresses are inherited from `GroupedSecurityGraph.Position`.
-/

namespace SigGolfCandidate.Hypertree.GroupedGraphPayload
open SigGolf Reference GroupedSecurityGraph

abbrev Address := SecurityGraph.Address

structure PrivateAnswers where
  bottom : BitVec 160 → BitVec 256
  upper : Fin 150 → BitVec 160 → Fin 26 → BitVec 256

def lowOrHigh (answer : BitVec 256) (chain : Fin 52) : Digest :=
...
def chainSource (privateAnswers : PrivateAnswers) (base : Fin 150)
    (leaf : BitVec 160) (chain : Fin 52) : Digest :=
...
def chainPayload (privateAnswers : PrivateAnswers) (labels : Labels)
  ...
  if hbottom : address.level.val = 0 ∧ address.leaf.val = 0 ∧
    ...
    if hleaf : address.leaf.val = 0 then
      if hchain : address.chain.val < 52 then
        if hstep : address.step.val < 5 then
          let base : Fin 150 := ⟨address.level.val - 10, by omega⟩
          let leaf : BitVec 160 := BitVec.ofNat 160 address.tree.toNat
          let chain : Fin 52 := ⟨address.chain.val, hchain⟩
  ...
  if hlevel : 10 ≤ address.level.val ∧ address.level.val < 160 then
    if hleaf : address.leaf.val = 0 ∧ address.chain.val = 0 ∧ address.step.val = 0 then
      ...
      (List.ofFn (fun chain : Fin 52 =>
        truncate (labels (upperChain base leaf chain ⟨4, by decide⟩)))).flatMap bytes
    else []
  else []

def groupBase (level : Nat) : Nat :=
  if level < 148 then 10 + 3 * ((level - 10) / 3)
  else 148 + 4 * ((level - 148) / 4)

theorem groupBase_low (group offset : Nat) (groupBound : group < 46)
    (offsetBound : offset < 3) :
    groupBase (10 + 3 * group + offset) = 10 + 3 * group := by
  have levelBound : 10 + 3 * group + offset < 148 := by omega
  simp only [groupBase, if_pos levelBound]
  omega

theorem groupBase_high (group offset : Nat) (groupBound : group < 3)
    (offsetBound : offset < 4) :
    groupBase (148 + 4 * group + offset) = 148 + 4 * group := by
  have levelBound : ¬148 + 4 * group + offset < 148 := by omega
...
theorem upper_chain_step_payload (privateAnswers : PrivateAnswers) (labels : Labels)
    (base : Fin 150) (leaf : BitVec 160) (chain : Fin 52) (step : Fin 4) :
...
theorem bottom_node_zero_children (tree : BitVec 160) :
  ...
  simp [nodeChildren, bottomNode, GroupedSecurityGraph.bottomNode]
...
end SigGolfCandidate.Hypertree.GroupedGraphPayload
