...
/-! Inlined from SigGolfCandidate.Hypertree.KeygenVerifyCountSecurity; its only importer was SigGolfCandidate.Hypertree.KeygenVerifyCountTree. -/
section
...
theorem upper_leaf_call_counted (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
  ...
  obtain ⟨final,steps,cycles,calls,run,hsteps,hcycles,hcalls,fpc,fsp,words,frame,eq⟩ :=
    upper_leaf_call_exact hash s level tree side base message signature small pc sp data nonzero aligned bound
