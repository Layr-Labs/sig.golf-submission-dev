...
/-! Inlined from SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Header67; its only importer was SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Index67. -/
section
/-! Construct the H2 chain header from level, chain, and step. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Header67
...
private theorem header_code :
    Keygen.instructionAt image 0x18f8 = some (.base (.ADDI .x10 .x0 2)) ∧
    Keygen.instructionAt image 0x18fc = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1900 = some (.base (.ADDI .x28 .x28 0)) ∧
    Keygen.instructionAt image 0x1904 = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1908 = some (.base (.SLLI .x11 .x11 8)) ∧
    Keygen.instructionAt image 0x190c = some (.base (.ADD .x10 .x10 .x11)) ∧
    Keygen.instructionAt image 0x1910 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1914 = some (.base (.ADDI .x28 .x28 0x30)) ∧
    Keygen.instructionAt image 0x1918 = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x191c = some (.base (.SLLI .x11 .x11 24)) ∧
    Keygen.instructionAt image 0x1920 = some (.base (.ADD .x10 .x10 .x11)) ∧
    Keygen.instructionAt image 0x1924 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1928 = some (.base (.ADDI .x28 .x28 0x38)) ∧
    Keygen.instructionAt image 0x192c = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1930 = some (.base (.SLLI .x11 .x11 32)) ∧
    Keygen.instructionAt image 0x1934 = some (.base (.ADD .x10 .x10 .x11)) ∧
    Keygen.instructionAt image 0x1938 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x193c = some (.base (.ADDI .x28 .x28 0)) ∧
    Keygen.instructionAt image 0x1940 = some (.base (.SD .x28 .x10 0)) := by decide

theorem header_steps (s : MachineState) (pc : s.pc = 0x18f8) :
  ...
  let s13 := execInstrBr s12 (.ADDI .x28 .x28 0x38)
  ...
  · rfl
  apply OrdinarySteps.step s7 s8 _ (.base (.ADDI .x28 .x28 0x30)) 11
  ...
  · rfl
  apply OrdinarySteps.step s12 s13 _ (.base (.ADDI .x28 .x28 0x38)) 6
  ...
  · have haddr : s13.getReg .x28 = 0x81038 := by
    ...
    change (if accessValid (s13.getReg .x28 + signExtend12 (0 : BitVec 12)) 8
      then some s14 else none) = some s14
    simp [haddr,signExtend12,accessValid,rangeValid,MEMORY_BYTES]
  ...
  · simpa [Keygen.fetch_at,s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,execInstrBr,pc] using c18
  · have haddr : s18.getReg .x28 = 0x80000 := by
      simp [s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17,s18,execInstrBr,MachineState.getReg_setReg_eq,
        MachineState.getReg_setReg_ne,signExtend12]
    change (if accessValid (s18.getReg .x28 + signExtend12 (0 : BitVec 12)) 8
      then some (headerState s) else none) = some (headerState s)
    simp [haddr,signExtend12,accessValid,rangeValid,MEMORY_BYTES]
  exact OrdinarySteps.refl _

theorem header_pc (s : MachineState) (pc : s.pc = 0x18f8) :
    (headerState s).pc = 0x1944 := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Header67

end

/-! Copy the three 64-bit Merkle address words into the paired H1 input. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Index67
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64
open SigGolfCandidate.Hypertree
set_option maxRecDepth 8192
...
private theorem index_code :
    Keygen.instructionAt image 0x1944 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1948 = some (.base (.ADDI .x28 .x28 8)) ∧
    Keygen.instructionAt image 0x194c = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1950 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x1954 = some (.base (.ADDI .x28 .x28 8)) ∧
    Keygen.instructionAt image 0x1958 = some (.base (.SD .x28 .x11 0)) ∧
    Keygen.instructionAt image 0x195c = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1960 = some (.base (.ADDI .x28 .x28 16)) ∧
    Keygen.instructionAt image 0x1964 = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1968 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x196c = some (.base (.ADDI .x28 .x28 16)) ∧
    Keygen.instructionAt image 0x1970 = some (.base (.SD .x28 .x11 0)) ∧
    Keygen.instructionAt image 0x1974 = some (.base (.LUI .x28 0x81)) ∧
    Keygen.instructionAt image 0x1978 = some (.base (.ADDI .x28 .x28 24)) ∧
    Keygen.instructionAt image 0x197c = some (.base (.LD .x11 .x28 0)) ∧
    Keygen.instructionAt image 0x1980 = some (.base (.LUI .x28 0x80)) ∧
    Keygen.instructionAt image 0x1984 = some (.base (.ADDI .x28 .x28 24)) ∧
    Keygen.instructionAt image 0x1988 = some (.base (.SD .x28 .x11 0)) := by decide

theorem index_steps (s : MachineState) (pc : s.pc = 0x1944) :
...
theorem index_pc (s : MachineState) (pc : s.pc = 0x1944) :
    (indexState s).pc = 0x198c := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperH2Index67
