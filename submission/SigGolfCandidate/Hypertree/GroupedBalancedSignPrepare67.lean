...
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignPrepare67
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 OracleComp Keygen
open SigGolfCandidate.Hypertree.Signing
open SigGolfCandidate.Hypertree.GroupedBalancedSignRandomizer67
...
theorem initializeState_block (s : MachineState) (pc : s.pc = 0x1000) :
    OrdinarySteps image s 4 (initializeState s) := by
  ...
  · have hp : s2.pc = 0x1008 := by simp [s1,s2,execInstrBr,pc]
    simp only [fetch,hp,image,GroupedBalancedSignImage67.image]; decide
  · rfl
  apply OrdinarySteps.step s3 _ _ (.base (.ADDI .x10 .x0 4)) 0
...
theorem messageCopyState_block (s : MachineState) (pc : s.pc = 0x1028) :
    OrdinarySteps image s 4 (messageCopyState s) := by
  ...
  · have hp : s.pc = 0x1028 := by simp [execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s1 s2 _ (.base (.LUI .x7 0x80)) 2
  · have hp : s1.pc = 0x102c := by simp [s1, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s2 s3 _ (.base (.ADDI .x7 .x7 0x40)) 1
  · have hp : s2.pc = 0x1030 := by simp [s1, s2, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s3 s4 _ (.base (.ADDI .x10 .x0 4)) 0
  · have hp : s3.pc = 0x1034 := by simp [s1, s2, s3, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  exact OrdinarySteps.refl _
...
theorem randomizerHeaderState_block (s : MachineState) (pc : s.pc = 0x1050) :
    OrdinarySteps image s 16 (randomizerHeaderState s) := by
  ...
  · have hp : s.pc = 0x1050 := by simp [execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  ...
  · have hp : s5.pc = 0x1064 := by simp [s1, s2, s3, s4, s5, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s6 s7 _ (.base (.ADDI .x28 .x28 8)) 9
  · have hp : s6.pc = 0x1068 := by simp [s1, s2, s3, s4, s5, s6, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s7 s8 _ (.base (.SD .x28 .x11 0)) 8
  · have hp : s7.pc = 0x106c := by simp [s1, s2, s3, s4, s5, s6, s7, execInstrBr, pc]
    simp only [fetch, hp]; decide
  ...
  · have hp : s8.pc = 0x1070 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s9 s10 _ (.base (.LUI .x28 0x80)) 6
  · have hp : s9.pc = 0x1074 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s10 s11 _ (.base (.ADDI .x28 .x28 16)) 5
  · have hp : s10.pc = 0x1078 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s11 s12 _ (.base (.SD .x28 .x11 0)) 4
  · have hp : s11.pc = 0x107c := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, execInstrBr, pc]
    simp only [fetch, hp]; decide
  ...
  · have hp : s12.pc = 0x1080 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s13 s14 _ (.base (.LUI .x28 0x80)) 2
  · have hp : s13.pc = 0x1084 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s14 s15 _ (.base (.ADDI .x28 .x28 24)) 1
  · have hp : s14.pc = 0x1088 := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s15 s16 _ (.base (.SD .x28 .x11 0)) 0
  · have hp : s15.pc = 0x108c := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, execInstrBr, pc]
    simp only [fetch, hp]; decide
...
theorem initializeState_mem (s : MachineState) (a : Word) :
    (initializeState s).getMem a = s.getMem a := by
  simp [initializeState,execInstrBr]

theorem initializeState_invariant (s : MachineState) (pc : s.pc = 0x1000) :
    CopyInvariant 0x1010 0x20 0x80020 4 4 (initializeState s) := by
...
theorem messageCopyState_invariant (s : MachineState) (pc : s.pc = 0x1028) :
    CopyInvariant 0x1038 0 0x80040 4 4 (messageCopyState s) := by
...
theorem randomizer_prepare (s : MachineState) (pc : s.pc = 0x1000) :
    ∃ ready, OrdinarySteps image s 72 ready ∧ ready.pc = 0x1090 ∧
  ...
  have secretKeyCode : CopyCode image 0x1010 := by decide
  have msgCode : CopyCode image 0x1038 := by decide
  obtain ⟨secretKey, secretKeyLoop, secretKeyInv, secretKeyOutput, secretKeyFrame⟩ := copy_all image 0x1010 secretKeyCode
    0x20 0x80020 4 (initializeState s) (initializeState_invariant s pc)
    (by decide) (by decide) (by decide) (by decide) (by decide)
  have secretKeypc : secretKey.pc = 0x1028 := by simpa [CopyInvariant] using secretKeyInv.2.2.1
  obtain ⟨msg, msgLoop, msgInv, msgOutput, msgFrame⟩ := copy_all image 0x1038 msgCode
    0 0x80040 4 (messageCopyState secretKey) (messageCopyState_invariant secretKey secretKeypc)
    (by decide) (by decide) (by decide) (by decide) (by decide)
  have msgpc : msg.pc = 0x1050 := by simpa [CopyInvariant] using msgInv.2.2.1
  ...
  · exact ordinary_trans image s _ _ 28 44
      (ordinary_trans image s _ _ 4 24 (initializeState_block s pc) secretKeyLoop)
      (ordinary_trans image secretKey _ _ 4 40 (messageCopyState_block secretKey secretKeypc)
        (ordinary_trans image (messageCopyState secretKey) _ _ 24 16 msgLoop (randomizerHeaderState_block msg msgpc)))
  ...
  · intro a mode pointer outside
    ...
    rw [randomizerHeaderState_mem, if_neg h3, if_neg h2, if_neg h1, if_neg h0,
      msgFrame, messageCopyState_mem, secretKeyFrame, initializeState_mem]
    · intro j hj
...
theorem entry_randomizer_trace (hash : Hash) (s : MachineState) (pc : s.pc = 0x1000) :
    ∃ ready final, Trace hash image s 108 123 1 2 final ∧ final.pc = 0x10d8 ∧
      (∀ i : Fin 12, ready.getMem (wordAddress 0x80000 i.val) = randomizerInputWord s i) ∧
      (∀ i : Fin 4, final.getMem (wordAddress 0x20060 i.val) =
        (hash (hashInput (GroupedBalancedSignRandomizer67.randomizerHashState ready))).extractLsb' (64 * i.val) 64) := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedSignPrepare67
