...
namespace SigGolfCandidate.Hypertree.GroupedBalancedVerifyByteReturn67
...
private theorem tail_pc (s : MachineState) (hlink : s.getReg .x1 = 0x1518) :
    (tailState s).pc = 0x1518 := by
  simp [tailState, execInstrBr, MachineState.getReg_setReg_ne,
    hlink, signExtend12]

theorem fullDecoder_return_pc (s : MachineState) (message : BitVec 128)
    (hlink : s.getReg .x1 = 0x1518) :
    (fullDecoderState s message).pc = 0x1518 := by
...
end SigGolfCandidate.Hypertree.GroupedBalancedVerifyByteReturn67
