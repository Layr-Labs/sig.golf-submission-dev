...
theorem leaf_loop_fast_core (hash : Hash) (s : MachineState) (level tree : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ cycles ∧
      cycles ≤ 11*calls+31*remaining+(if start = 0 then 49 else 0) ∧
  ...
  | succ remaining ih =>
    ...
    · change preCycles ≤ 11*preCalls+31+(if start=0 then 49 else 0) at preCycleBound
      omega
...
theorem recover_all_chains_fast_core (hash : Hash) (s : MachineState)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ cycles ∧ cycles ≤ 11*calls+1475 ∧ calls ≤ 308 ∧
