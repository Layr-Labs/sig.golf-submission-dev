...
theorem leaf_loop_fast_core (hash : Hash) (s : MachineState) (level tree : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ cycles ∧
      cycles ≤ 11*calls+30*remaining+(if remaining = 0 then 0 else 1)+(if start = 0 then 49 else 0) ∧
  ...
  | succ remaining ih =>
    ...
    · change preCycles ≤ 11*preCalls+30+(if start+1=46 then 1 else 0)+(if start=0 then 49 else 0)
        at preCycleBound
      split_ifs at * <;> omega
...
theorem recover_all_chains_fast_core (hash : Hash) (s : MachineState)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ cycles ∧ cycles ≤ 11*calls+1430 ∧ calls ≤ 308 ∧
