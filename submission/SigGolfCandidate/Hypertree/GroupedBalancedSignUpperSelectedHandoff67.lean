import SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedPriorFrame67
...
/-! Inlined from SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedPriorHandoff67; its only importer was SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedHandoff67. -/
section
/-! At the chosen upper leaf, the complete WOTS program materializes the
functional signature values before H3 compression. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedPriorHandoff67
...
theorem selected_handoff_prior (hash : Hash) (secretKey : SecretKey)
    ...
    ∃ (b n c : Nat) (ready : MachineState),
      ...
      (∀ a : Word, a.toNat<b → ready.getMem a=s.getMem a) ∧
      n ≤ 36122 ∧ c ≤ 41281 := by
  ...
  have startSelected : start.getMem 0x810e0=start.getMem 0x810e8 := by
    rw [GroupedBalancedSignUpperChainZero67.zero_frame s 0x810e0
      ...
      (by decide),inv.data.count,inv.data.chosen]
  have startDigits : ∀ j : GroupedBalancedUpperTree67.ChainMixed,
    ...
    rw [GroupedBalancedSignUpperWitnessSlots67.digit_region_frame
      s start j (by
        ...
        exact GroupedBalancedSignUpperChainZero67.zero_frame s a (by
          ...
          omega))]
    exact digits j
  have startInv := GroupedBalancedSignUpperSelectedPriorFrame67.initial hash
    secretKey treeBase (leafBase+selected) b message start zeroPc
      zeroCounter startKey startSelected startDigits
  ...
  have startAddress : ∀ w : Fin 3,
      start.getMem (Signing.wordAddress 0x81008 w.val)=
        (BitVec.ofNat 192 (leafBase+selected)).extractLsb'
          (64*w.val) 64 := by
    intro w
    rw [GroupedBalancedSignUpperChainZero67.zero_frame s _
      (by fin_cases w <;> decide)]
  ...
  have startWitness : start.getMem 0x810f0=BitVec.ofNat 64 b := by
    rw [GroupedBalancedSignUpperChainZero67.zero_frame s 0x810f0
      (by decide)]
    exact bWord
  obtain ⟨n,c,ready,path,readyPc,witnessWords,endpointWords,
    controlFrame,readySp,keyWords,lowFrame,priorFrame,nBound,cBound⟩ :=
    GroupedBalancedSignUpperSelectedPriorFrame67.run_all_copy hash secretKey
      treeBase (leafBase+selected) b message start baseBound startLevel
      startAddress startWitness bLower bUpper bAlign startInv
  ...
  refine ⟨b,4+n,4+c,ready,bWord,bLower,bUpper,bAlign,full,
    readyData,witnessWords,?_,?_,?_,?_,by omega,by omega⟩
...
#print axioms selected_handoff_prior
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedPriorHandoff67

end

/-! At the chosen upper leaf, the complete WOTS program materializes the
functional signature values before H3 compression. -/
namespace SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedHandoff67
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64
open SigGolfCandidate.Hypertree
open GroupedBalancedSignUpperLeafH3Data67
open GroupedBalancedSignBottomStackSlots67
...
theorem selected_handoff (hash : Hash) (secretKey : SecretKey)
    (treeBase leafBase height selected witnessBase : Nat)
    (message : Reference.Digest)
    (s : MachineState)
    (hh : height=3 ∨ height=4)
    ...
    rw [GroupedBalancedSignUpperWitnessSlots67.digit_region_frame
      s start j (by
        ...
        exact GroupedBalancedSignUpperChainZero67.zero_frame s a (by
          ...
          omega))]
    exact digits j
  have startInv := GroupedBalancedSignUpperWotsSelectedFold67.initial hash
    secretKey treeBase (leafBase+selected) b message start zeroPc
      zeroCounter startKey startSelected startDigits
  have startLevel : start.getMem 0x81000=BitVec.ofNat 64 treeBase := by
    rw [GroupedBalancedSignUpperChainZero67.zero_frame s 0x81000
  ...
  have startWitness : start.getMem 0x810f0=BitVec.ofNat 64 b := by
    ...
    exact bWord
  obtain ⟨n,c,ready,path,readyPc,witnessWords,endpointWords,
    controlFrame,readySp,keyWords,lowFrame⟩ :=
    GroupedBalancedSignUpperWotsSelectedFold67.run_all_copy hash secretKey
      treeBase (leafBase+selected) b message start baseBound startLevel
      startAddress startWitness bLower bUpper bAlign startInv
  have control (a : Word) (high : 0x81000≤a.toNat)
      (ne30 : a≠0x81030) (ne38 : a≠0x81038) :
...
theorem selected_leaf (hash : Hash) (secretKey : SecretKey)
    (treeBase leafBase height selected witnessBase : Nat)
    (message : Reference.Digest)
    (s : MachineState)
    (hh : height=3 ∨ height=4)
    ...
    (digits : ∀ j : GroupedBalancedUpperTree67.ChainMixed,
      s.getByte (digitAddress j)=
        BitVec.ofNat 8
          (GroupedBalancedUpperTree67.digit message j).val) :
    ∃ (b n c : Nat) (next : MachineState),
      s.getMem 0x810f0=BitVec.ofNat 64 b ∧
      0x20060≤b ∧ b+16*67≤0x80000 ∧ b%8=0 ∧
      Trace hash image s n c 248 265 next ∧
      GroupedBalancedSignUpperLeafFold67.Inv hash secretKey treeBase
        leafBase height selected witnessBase (selected+1) next ∧
      (∀ j : GroupedBalancedUpperTree67.ChainMixed, ∀ w : Fin 2,
        next.getMem (witnessSlot b j w)=
          (GroupedBalancedUpperTree67.signValues hash secretKey treeBase
            (leafBase+selected) message j).extractLsb'
              (64*w.val) 64) ∧
      (∀ a : Word, GroupedBalancedSignUpperLeafPersistent67.Persistent a →
        next.getMem a=s.getMem a) ∧
      (∀ a : Word, a.toNat<b → next.getMem a=s.getMem a) ∧
      next.getReg .x2=s.getReg .x2 ∧
      n ≤ 36188 ∧ c ≤ 41490 := by
  obtain ⟨b,n,c,ready,bWord,bLower,bUpper,bAlign,wotsTrace,
    readyData,witnessWords,readyCurrent,readySp,readyFrame,readyPrior,
    nBound,cBound⟩ :=
    GroupedBalancedSignUpperSelectedPriorHandoff67.selected_handoff_prior
      hash secretKey treeBase leafBase height selected
      witnessBase message s hh baseBound hi inv digits
  obtain ⟨next,h3Trace,after,frame,nextSp,nextLow,_nextDigits⟩ :=
    GroupedBalancedSignUpperLeafH3Data67.one_leaf hash secretKey treeBase
      leafBase height selected selected ready hh aligned bound hi readyData
  have countSmall : (ready.getMem 0x810e0).toNat<1024 := by
    rw [readyData.count]
    simp only [BitVec.toNat_ofNat]
    have h16 : selected<16 := by rcases hh with rfl | rfl <;> omega
    rw [Nat.mod_eq_of_lt (by omega : selected<2^64)]
    omega
  ...
  have nextKey : ∀ j : Fin 4,
    ...
    intro j
    rw [nextLow _ (by fin_cases j <;> decide),
      readyFrame _ (Or.inl (by fin_cases j <;> decide))]
    exact inv.keyWords j
  have nextInv : GroupedBalancedSignUpperLeafFold67.Inv hash secretKey
      treeBase leafBase height selected witnessBase (selected+1) next :=
    ⟨after,nextHeight,nextWitness,nextCurrent,nextStack,nextKey⟩
  have nextWitnessWords : ∀ j : GroupedBalancedUpperTree67.ChainMixed,
      ∀ w : Fin 2,
      next.getMem (witnessSlot b j w)=
        (GroupedBalancedUpperTree67.signValues hash secretKey treeBase
          (leafBase+selected) message j).extractLsb'
            (64*w.val) 64 := by
    intro j w
    rw [nextLow _ (GroupedBalancedSignUpperWitnessSlots67.slot_bound
      b j w bLower bUpper).2]
    exact witnessWords j w
  have nextFrame : ∀ a : Word,
  ...
  have nextPrior : ∀ a : Word, a.toNat<b →
      next.getMem a=s.getMem a := by
    intro a before
    rw [nextLow a (by omega : a.toNat<0x80000)]
    exact readyPrior a before
  refine ⟨b,n+66,c+209,next,bWord,bLower,bUpper,bAlign,?_,
    nextInv,nextWitnessWords,nextFrame,nextPrior,?_,by omega,by omega⟩
  · simpa [Nat.add_assoc] using wotsTrace.trans h3Trace
  · rw [nextSp,readySp]

#print axioms selected_handoff
#print axioms selected_leaf
end SigGolfCandidate.Hypertree.GroupedBalancedSignUpperSelectedHandoff67
