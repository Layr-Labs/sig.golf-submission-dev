...
theorem chain_step (hash : Hash) (s : MachineState) (level tree step : Nat)
    ...
    ∃ final, Trace hash verify s 80 87 1 1 final ∧ final.pc = 0x14ec ∧
  ...
  obtain ⟨hashed, core, hashedPC, valueOut, ra, sp, frame⟩ := VerifyChainDirect.compute verify hash 0x1500 verify_chain_code
  ...
  have keep (a : Word)
      ...
      (hv : ∀ i : Fin 4, a ≠ wordAddress 0x80510 i.val) : hashed.getMem a = s.getMem a := by
    rw [frame a hi hv, check_mem]
  have nextLevel : hashed.getMem 0x80400 = s.getMem 0x80400 := keep _ (by decide) (by decide)
  have nextLeaf : hashed.getMem 0x80428 = s.getMem 0x80428 := keep _ (by decide) (by decide)
  have nextChain : hashed.getMem 0x80430 = s.getMem 0x80430 := keep _ (by decide) (by decide)
  have nextStep : hashed.getMem 0x80438 = s.getMem 0x80438 := keep _ (by decide) (by decide)
  ...
  · intro a outside
    rw [increment_mem, if_neg outside.2.2.2, keep a (fun i => outside.1 ⟨i.val, by omega⟩) outside.2.2.1]
